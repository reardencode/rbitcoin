...
**0.8 (plan):** drop-in **Bitcoin Core JSON-RPC + mempool/electrs or
Blockstream electrs** so nginx `/api/` and `CORE_RPC` can retire bitcoind +
electrs. mempool.space **Node `/api/v1/`** (MariaDB, cubes, mining, lightning)
stays their process. Address-prefix search is **not** in 0.8. Plan:
[`docs/esplora-mempool-backend.md`](./docs/esplora-mempool-backend.md)
(**Q-68**). Until that plan’s last step, shipped HTTP is wallet-exact Esplora
(no `/internal/*`, Bearer RPC, TCP Esplora only).
...
| **Non-goal / never** | — | Address-prefix search, Liquid/assets. mempool.space `/api/v1/` catalogue stays their Node. **0.8** `/internal/*` electrs-fork bulk routes: [`docs/esplora-mempool-backend.md`](./docs/esplora-mempool-backend.md) |
...
lightning). **0.8 plan** claims Core + electrs drop-in (except prefix) after
its last step: [`docs/esplora-mempool-backend.md`](./docs/esplora-mempool-backend.md).
