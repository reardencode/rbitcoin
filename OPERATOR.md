...
**0.8** Core+electrs drop-in (mempool.space nginx `/api/`):
...
(exact address/scripthash, tx/block by id, broadcast). **0.8 (plan)** adds
mempool/electrs `/internal/*` bulk routes and unix listen so this process can
replace bitcoind+electrs behind mempool.space nginx `/api/`
([`docs/esplora-mempool-backend.md`](./docs/esplora-mempool-backend.md)).
...
| Unknown / non-goal | **404** (address-prefix; Liquid). `GET /block-template` is 404 unless `--esplora-block-template`. **0.8** `/internal/*`: [`docs/esplora-mempool-backend.md`](./docs/esplora-mempool-backend.md) |
