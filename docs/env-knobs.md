...
| `RBITCOIN_SH_MATERIALIZE` | unset = catalog k-way | `unsorted-shards`: one Class A pass writes unsorted `scripthash.unsorted/NN`, then in-place unique-sort + seal (`~2 GiB`/pack worker). Unset keeps catalog k-way |
