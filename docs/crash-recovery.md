...
- Scripthash: Direct IBD **defers** SH (no memtable, no confirm enqueue). After the horizon, Class A recollect spills catalog runs (`SEAL` = resume floor) then FullCold / ColdResume. A **durable head** on restart stays Tip: write-behind / `recover_sh_writebehind` fills any HWM lag; leftover `scripthash.runs` are discarded (not WarmOnly-merged).
  ...
  - **`RBITCOIN_SH_MATERIALIZE=unsorted-shards`:** one Class A pass writes unsorted `scripthash.unsorted/NN` (concurrent pwrite). Incomplete collect (no `DONE`) restarts the pass. `DONE` + sealed heads: pack only unsealed files (in-place unique-sort ~2 GiB/worker). After all shards seal, the unsorted dir is removed.
