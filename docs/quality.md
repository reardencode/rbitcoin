...
| **Q-41** | Keep rank 1. 66 → **67** `run`. 200 skips; `rpc-missing` 21 + `core-log` 21. `p2p_timeouts` handshake timeout honors `-peertimeout` + `setmocktime`. `p2p_leak` hides `addrFrom`, skips pre-verack pong, gates BIP155 to nVersion ≥70016, disconnects obsolete &lt;31800. |
...
| **—** | Core functional `run` set | **67** unmodified v31.1 scripts (was 62 at last reaudit, 9 at first green). Remaining growth is **Q-41** |
