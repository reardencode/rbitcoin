...
|-----:|----|------|-----|-----------------|
| 1 | **Q-30** | Continuous differential fuzz | reliability | A nightly/weekly job that feeds BIP324 + header/block (and script) wire. Crashes → `docs/external_findings/` + named regression. **Today: `fuzz/` `block_wire` + `v2_contents` (ASan), `v2_session` (live Core v2 handshake), `cmpct_differential` (empty-mempool compact missing indexes vs Core `getblocktxn`), and three Core `submitblock` diffs vs official v31.1 `bitcoind` on nightly `fuzz.yml` (not a required PR check).** Still missing two-node compact reorg P2P and script-mutating vs live Core (JSON corpora are static vectors). Findings 001–023 came from external review + fuzzamoto — that is not a substitute for the in-tree job. |
...
| ID | Verdict |
|----|---------|
| **Q-30** | Keep rank 1. Three Core `submitblock` diffs + ASan `block_wire` / `v2_contents` + live Core `v2_session` + compact reconstruct vs `getblocktxn` landed; still not two-node compact reorg P2P or script-mutating vs live Core. External fuzzamoto + finding 023 are not a substitute job. |
...
| Fuzz | **ASan `block_wire` + `v2_contents`; live Core `v2_session` + `cmpct_differential`; three Core `submitblock` diffs** (breadth still Q-30) |
...
| Adversarial / findings | Medium–Strong | **001–023** closed; in-tree Core `submitblock` diffs + ASan `block_wire` / `v2_contents` + live Core `v2_session` + compact reconstruct vs `getblocktxn`; breadth is **Q-30**. Core functional is the active surface program (**Q-41**) |
