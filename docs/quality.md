...
| 7 | **Q-68** | Hostname `--connect` / `addnode` resolve at dial + retry until live | `addnode add` and `--connect` store host strings; resolve each dial (`localhost` / missing port uses the network P2P default); redial until a live session without a second RPC. Seconds-scale backoff. `--connect` must not enable seed redial. Owner: [`rpc.md`](./rpc.md) / [`COMPAT.md`](../COMPAT.md) network. |
