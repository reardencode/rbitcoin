...
| **Q-57** | **Closed.** Dirty-epoch CAS on Class C `flush_dirty`; fuse8 `decode_body` closed fingerprint length; spender overflow hops ≤ `spenders.count()`; sidecar tmp+`sync_all`+rename; `list_runs` scan vs `list_runs_gc(&RunsIoGuard)`. |
...
| ID | Item | Resolution |
|----|------|------------|
| **Q-57** | Store publish / Class C flush / sidecar | `published_meta` Acquire. `ArrayTable` / `StrongTxTable` `flush_dirty` packed dirty-epoch (`0`=clean; snapshot under read; CAS `e0→0`). fuse8 `decode_body` refuses fingerprint arrays shorter than `hash_of_hash` geometry (`contains` unchanged). Leftover fuse8 v1 refuses open. `for_each_spender_create` hops ≤ `spenders.count()`. Seal/install of `meta` / `.mphf` / SH `.idx` is tmp + `sync_all` then rename. `list_runs` is a scan; orphan unlink is `list_runs_gc(&RunsIoGuard)` (open-time SH `key_len` check does not delete). |
