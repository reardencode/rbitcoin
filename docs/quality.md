...
| ID | Verdict |
|----|---------|
| **Q-30** | Keep rank 1. Height-1, spend-pad, and 2-block fork differentials vs official v31.1 `bitcoind` landed; still not BIP324 / script-wire / compact-reorg P2P. External fuzzamoto + finding 023 are not a substitute job. |
...
| Fuzz | **height-1 + spend-pad + 2-block fork differential** (`block_wire` ASan + three Core `submitblock` targets; breadth still Q-30) |
...
| Adversarial / findings | Medium–Strong | **001–023** closed; in-tree height-1 / spend-pad / 2-block fork diffs vs Core v31.1; breadth is **Q-30**. Core functional is the active surface program (**Q-41**) |
