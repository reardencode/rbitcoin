...
**Last reaudit:** 2026-09-15. Schema **24**. Core functional **66** `run` /
**201** `skip`. Findings **001–023** fixed. Nightly fuzz **20** jobs.
Previous: 2026-09-13.
...
|-----:|----|------|-----------------|
| 1 | **Q-41** | Grow Core functional `run` set | Inventory `run` covers claimed wallet-client / P2P / mempool / buried-activation. **66 run / 201 skip** (19 `rpc-missing`, 18 `core-log`, 68 `no-wallet`). COMPAT leftovers are `rpc-dialect`, not `rpc-missing`. `run` must hit node production (not only shim argv / dummy `blk*.dat` / Core decode dialect). Next `run`: `p2p_permissions`. `mempool_accept` stays skip (`policy-libre`). Unlabeled PRs stay cargo-only. Owner: [`core-functional.md`](./core-functional.md). |
