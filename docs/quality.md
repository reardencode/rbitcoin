...
| **Q-60** | **Closed.** Prefill monotonic + `held_seq` + WS `spawn_blocking` + outbound asmap/prefix diversity + `requested_blocks` 10s expire. AddrMan 4096 (`MAX_ADDR_MAN`) on learn/`peers` load/`addpeeraddress`; `--connect`/DNS `add` evicts oldest new and may exceed when only tried remain; `merge_from` trims. `announced_wtx` / `from_this_peer` recency FIFO-roll at 50k. `cmpct_fills` decrements on fail/getdata expire/unregister (Accepted still `clear_cmpct_fill`). |
