...
|-----:|----|------|-----|-----------------|
| 1 | **Q-30** | Continuous differential fuzz | reliability | A nightly/weekly job that feeds BIP324 + header/block (and script) wire. Crashes → `docs/external_findings/` + named regression. **Today: `fuzz/` `block_wire` + `v2_contents` (ASan), `v2_session` (live Core v2 handshake + fuzzed app contents, ASan), and three Core `submitblock` diffs (`block_differential` / `block_spend_differential` / `block_fork_differential`) vs official v31.1 `bitcoind`, `--sanitizer none`, on nightly `fuzz.yml` (not a required PR check).** Still missing compact-reorg P2P. Script-mutating vs Core is skipped (JSON corpora already no allowlist). Findings 001–023 came from external review + fuzzamoto — that is not a substitute for the in-tree job. |
...
| ID | Verdict |
|----|---------|
| **Q-30** | Keep rank 1. Three Core `submitblock` diffs + ASan `block_wire` / `v2_contents` + live Core `v2_session` landed; still not compact-reorg P2P. Script-mutating vs Core is skipped. External fuzzamoto + finding 023 are not a substitute job. |
...
| Fuzz | **ASan `block_wire` + `v2_contents`; live Core `v2_session`; three Core `submitblock` diffs** (breadth still Q-30) |
...
| Adversarial / findings | Medium–Strong | **001–023** closed; in-tree Core `submitblock` diffs + ASan `block_wire` / `v2_contents` + live Core `v2_session`; breadth is **Q-30**. Core functional is the active surface program (**Q-41**) |
