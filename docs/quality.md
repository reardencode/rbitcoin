...
Former `algo-review.md` items that are still worth doing are **Q-58** and
**Q-60**. **Q-57** and **Q-59** are Completed.
...
| **Q-57** | **Closed.** Dirty-epoch CAS on Class C `flush_dirty` (wrap of `u64::MAX` stays dirty); fuse8 `decode_body` closed fingerprint length; spender overflow hops ≤ `spenders.count()`; sidecar tmp+`sync_all`+rename; `list_runs` scan vs `list_runs_gc(&RunsIoGuard)` (leftover **count** and open-time SH `key_len` do not unlink). |
| **Q-58** | Keep rank 2. `evict_to_budget` no-op break landed. `persist_all` still meta/slots then body. |
...
| **Q-60** | Keep rank 3. Prefill monotonic + `held_seq` + WS `spawn_blocking` + outbound asmap/prefix diversity landed. IBD getaddr inject 4096 + last-resort eviction. AddrMan new/tried buckets and `announced_wtx` roll still open. |
| **Q-48** | Keep, rank 4. Waits on rust-bitcoin (**RB-007**). |
| **Q-31** | Keep rank 5. Feeds nightly fuzz corpora; not blocking operators. |
| **R-10** | Keep rank 6. Unnamed peels wait. Largest production file is now `electrum/server` **5.8k** (tweaks subscribe). `sorted_run` peeled to **1.3k**. |
...
| ID | Item | Resolution |
|----|------|------------|
| **Q-57** | Store publish / Class C flush / sidecar | `published_meta` Acquire. `ArrayTable` / `StrongTxTable` `flush_dirty` packed dirty-epoch (`0`=clean; snapshot under read; CAS `e0→0`; wrap of `u64::MAX` stays dirty). fuse8 `decode_body` refuses fingerprint arrays shorter than `hash_of_hash` geometry (`contains` unchanged). Leftover fuse8 v1 refuses open. `for_each_spender_create` hops ≤ `spenders.count()`. Seal/install of `meta` / `.mphf` / SH `.idx` is tmp + `sync_all` then rename. `list_runs` is a scan; orphan unlink is `list_runs_gc(&RunsIoGuard)` (open-time SH `key_len` and leftover-run count do not delete). |
...
| Audience | Read |
|----------|------|
| Next quality slice | **Open**, rank 1 (**Q-41** Core functional `run` set). **Q-61** named extracts are Completed. Folded leftovers still Open: **Q-58**, **Q-60**. Residual peels: **R-10**. Next unused Q-id is **Q-62** |
