...
| ID | Verdict |
|----|---------|
| **Q-30** | Keep rank 1. Three Core `submitblock` diffs + ASan `block_wire` / `v2_contents` landed; still not live Core v2 session / script-wire / compact-reorg P2P. External fuzzamoto + finding 023 are not a substitute job. |
...
| Fuzz | **ASan `block_wire` + `v2_contents`; three Core `submitblock` diffs** (breadth still Q-30) |
...
| Adversarial / findings | Medium–Strong | **001–023** closed; in-tree Core `submitblock` diffs + ASan `block_wire` / `v2_contents`; breadth is **Q-30**. Core functional is the active surface program (**Q-41**) |
