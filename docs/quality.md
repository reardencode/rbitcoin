...
| **Q-41** | Keep rank 1. 62 → **65** `run`. 202 skips; `rpc-missing` 21 + `core-log` 23 are the growth matching claimed surface. `rpc_createmultisig` 3-node `generate(149)` `sync_blocks`: unanswered catch-up `getdata` expires (10s) so serve-cap skips cannot stick `asked_blocks`. |
