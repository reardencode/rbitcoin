...
| **Q-60** | **Closed.** Prefill monotonic + `held_seq` + WS `spawn_blocking` + outbound asmap/prefix diversity + `requested_blocks` 10s expire. AddrMan 4096 (`MAX_ADDR_MAN`) on learn/`peers` load/`addpeeraddress`; `--connect`/DNS `add` evicts oldest new and may exceed when only tried remain; `merge_from` trims. `announced_wtx` / `from_this_peer` insertion-order FIFO-roll at 50k. `cmpct_fills` decrements on fail/getdata expire/unregister (Accepted still `clear_cmpct_fill`). |
...
| **Q-60** | P2P caps + compact reconstruction | Compact prefill monotonic; held FIFO; getdata 10s expire; outbound asmap/prefix diversity. AddrMan 4096 on learned/`peers` load/`addpeeraddress` (evict incompat → failed → oldest new); `--connect`/DNS `add` may exceed when only tried remain; `merge_from` trims to 4096. `announced_wtx` / `from_this_peer` insertion-order FIFO-roll at 50k. `cmpct_fills` decrement on fail/expire/unregister (Accepted `clear_cmpct_fill`). |
