...
| 2 | **Q-57** | Store publish / Class C flush / sidecar | store | `VarTable::published_meta` loads count/end `Acquire` (ARM cannot tear the pair). `ArrayTable` / `StrongTxTable` `flush_dirty` cannot lose a `set` in the write window (clear dirty then snapshot, or equivalent). fuse8 `decode_body` fails closed instead of indexing fingerprints OOB. Leftover fuse8 v1 refuses open. Spender overflow walk bounded by `spenders.count()`. Sidecar meta / `.mphf` / SH `.idx` do not rename an unsynced empty file into place. `sorted_run` orphan GC cannot delete a live run (lock is a type, not a comment). **Today: `published_meta` seqlock Acquire landed.** `BinaryFuse8::contains` still indexes `fingerprints` without a closed length check. |
| 3 | **Q-58** | Mempool persist order + eviction | mempool | `persist_all` writes body before claiming LIVE slots. Known-parent out-of-range vout hard-rejects (not orphan-forever). `worst_chunk` rate-tie does not strand descendants. `evict_to_budget` no-op iterations break (no spin). **Today: `evict_to_budget` breaks when a pass removes 0. Known-parent OOB vout is `MissingPrevout` (not orphan).** `persist_all` still writes meta/slots then body. |
| 4 | **Q-59** | RPC / CLI honesty | ops | `submitblock` matches [`rpc.md`](./rpc.md) / COMPAT (all networks) or those docs say regtest-only. `gettxout include_mempool` hides mempool-spent confirmed outs. `sendrawtransaction` / `submitpackage` enforce or reject `maxfeerate` / `maxburnamount`. Conf `milestone=0` is not overwritten by the network default. `--minrelaytxfee` parse failure is an error (negatives rejected). `getmininginfo` `blockmintxfee` uses a feerate formatter. `getnetworkhashps` is not a dummy ~2 hashes/block (or is labeled). JSON-RPC batch is bounded under the work permit. **Today: `submitblock` is all-networks via `ChainHub`. `--minrelaytxfee` garbage/negatives fail start. `getnetworkhashps` is labeled dummy 2-work-per-block. `gettxout` does not hide mempool-spent confirmed outs. JSON-RPC array batch is unbounded. `getmininginfo` `blockmintxfee` formatter still Open.** |
| 5 | **Q-60** | P2P caps + compact reconstruction | p2p | Compact-block prefilled indexes are strictly increasing and in-bounds. AddrMan has tried/new caps. `cmpct_fills` / `requested_blocks` prune on abandon/timeout. `announced_wtx` rolls instead of clear-all INV burst. Pending/held eviction is FIFO (`held_seq`), not `HashMap::keys().next()`. Esplora WS mempool-announce store IO uses `spawn_blocking` like REST (**landed**). IBD `disconnect_to` skips cloning the losing branch when there is no mempool. Outbound IBD/tip-follow diversity uses Core asmap ASN buckets when `ip_asn.dat` is loaded, else IPv4 `/16` / IPv6 `/32` (**landed**). **Today: `prefilled_indexes_ok` + `held_seq` FIFO + WS `spawn_blocking` + `requested_blocks` 10s expire + outbound asmap/prefix diversity landed.** `announced_wtx` still `clear()` at 50k. IBD getaddr inject is capped at 4096 with last-resort eviction; AddrMan HashMap is still unbounded for seeds / `peers` file. |
| 6 | **Q-48** | BIP331 rust-bitcoin package types | interop | Native BIP331 `NetworkMessage` when rust-bitcoin exposes it (**RB-007**). Packages today are RPC `submitpackage` / Esplora `POST /txs/package` only — no private P2P command. Blocked upstream — ranked below unblocked ops work. **After this:** Electrum 1.6 then 1.7 (`protocol_max` bump in the same work) — [`COMPAT.md`](../COMPAT.md) § Protocol versions |
| 7 | **Q-31** | Hermetic tip fixtures | ops | Frozen signet/mainnet tip packs for offline consensus/Electrum regression (no live API). **Fuzz corpora** now merge tiny `signet_block_*.bin` / `mainnet_block_290329.bin` (remined onto regtest). Electrum hermetic packs still Open. |
| 8 | **R-10** | Residual god-files | code | Peel **only** when a higher row needs a seam. Unnamed line-count peels still wait. Do not split `interpreter.rs` opcode `match` or io_uring machines. Production leftover (2026-09-03 `wc -l`): `electrum/server` **5.8k**, `query/lib` **5.1k**, `chain` **3.9k**, `rpc/methods` **3.6k**, `scripthash` **3.5k**, `store` **3.4k**, `peer` **2.9k**, `ibd/perf_log` **2.9k**, `interpreter` **2.7k**. `sorted_run` peeled to **1.3k**. **Q-54** may need a seam if a cap rule cannot match a god-file. **Q-61** named extracts are Completed. |
...
| **Q-59** | Keep rank 4. `submitblock` all-networks and labeled `getnetworkhashps` dummy landed. Residual: `gettxout` mempool-spent confirmed, unbounded JSON-RPC batch, `maxfeerate`/`maxburnamount`, `getmininginfo` `blockmintxfee` formatter. |
| **Q-60** | Keep rank 5. Prefill monotonic + `held_seq` + WS `spawn_blocking` + outbound asmap/prefix diversity landed. IBD getaddr inject 4096 + last-resort eviction. AddrMan new/tried buckets and `announced_wtx` roll still open. |
| **Q-48** | Keep, rank 6. Waits on rust-bitcoin (**RB-007**). |
| **Q-31** | Keep rank 7. Feeds nightly fuzz corpora; not blocking operators. |
| **R-10** | Keep rank 8. Unnamed peels wait. Largest production file is now `electrum/server` **5.8k** (tweaks subscribe). `sorted_run` peeled to **1.3k**. |
...
| Operator honesty | Strong | CLI primary; chaininfo disk/progress are real (Q-47); schema 20 refuse names wipe dirs; leftover hop-dump no longer dropped by the next resolve. Residual Q-59: `gettxout` mempool-spent, unbounded JSON-RPC batch, `maxfeerate` |
...
| Dead / stub surface | Strong | Node RPC is a real subset; chaininfo disk/progress real (Q-47). `getnetworkhashps` is labeled dummy 2-work-per-block — **Q-59** |
...
| Audience | Read |
|----------|------|
| Next quality slice | **Open**, rank 1 (**Q-41** Core functional `run` set). **Q-61** named extracts are Completed. Folded store/mempool/RPC/P2P leftovers: **Q-57–Q-60**. Residual peels: **R-10**. Next unused Q-id is **Q-62** |
