...
Former `algo-review.md` items that are still worth doing are **Q-60**.
**Q-57**, **Q-58**, and **Q-59** are Completed.
...
| 4 | **Q-31** | Hermetic tip fixtures | ops | Frozen signet/mainnet tip packs for offline consensus/Electrum regression (no live API). **Fuzz corpora** already merge tiny `signet_block_*.bin` / `mainnet_block_290329.bin` (remined onto regtest). Electrum hermetic packs still Open. |
| 5 | **R-10** | Residual god-files | code | Peel **only** when a higher row needs a seam. Unnamed line-count peels still wait. Do not split `interpreter.rs` opcode `match` or io_uring machines. Production leftover (2026-09-03 `wc -l`): `electrum/server` **5.8k**, `query/lib` **5.1k**, `chain` **3.9k**, `rpc/methods` **3.6k**, `scripthash` **3.5k**, `store` **3.4k**, `peer` **2.9k**, `ibd/perf_log` **2.9k**, `interpreter` **2.7k**. `sorted_run` peeled to **1.3k**. **Q-54** may need a seam if a cap rule cannot match a god-file. **Q-61** named extracts are Completed. |
...
| **Q-58** | **Closed.** Admit `persist_all` is body then slots then meta. Compact installs packed images via tmp+`sync_all`+rename (open finishes `slots.tmp`). `evict_worst_chunk_once` uses `remove_txid_tree`. |
...
| **Q-60** | Keep rank 2. Prefill monotonic + `held_seq` + WS `spawn_blocking` + outbound asmap/prefix diversity landed. IBD getaddr inject 4096 + last-resort eviction. AddrMan new/tried buckets and `announced_wtx` roll still open. |
| **Q-48** | Keep, rank 3. Waits on rust-bitcoin (**RB-007**). |
| **Q-31** | Keep rank 4. Feeds nightly fuzz corpora; not blocking operators. |
| **R-10** | Keep rank 5. Unnamed peels wait. Largest production file is now `electrum/server` **5.8k** (tweaks subscribe). `sorted_run` peeled to **1.3k**. |
...
| **Q-58** | Mempool persist order + eviction | Admit `persist_all` writes body then LIVE slots then meta. Compact writes `tx.body.tmp`/`slots.tmp`, `sync_all` both, rename body then slots (open finishes a leftover `slots.tmp`). Known-parent OOB vout is `MissingPrevout`. `evict_to_budget` no-op break. `evict_worst_chunk_once` uses `remove_txid_tree` so a parent-only worst chunk cannot leave a child. |
...
| Audience | Read |
|----------|------|
| Next quality slice | **Open**, rank 1 (**Q-41** Core functional `run` set). **Q-61** named extracts are Completed. Folded leftovers still Open: **Q-60**. Residual peels: **R-10**. Next unused Q-id is **Q-62** |
