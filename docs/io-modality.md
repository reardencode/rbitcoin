...
| IBD **getdata serve** reconstruct | FdOnly `txout.idx` / `inwit.idx` ranges for a contiguous `header_txs` run | libc sequential **span** pread of `txout.body` + `inwit.body` (not confirm `idx_body_pipeline`) |
