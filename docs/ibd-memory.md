...
| **SH unsorted-shards collect / pack** | Collect: nCPU (`RBITCOIN_SH_RECOLLECT_WORKERS` override). Pack: min(CPUs, free RAM / 2 GiB); `RBITCOIN_SH_MERGE_WORKERS` override. Env `RBITCOIN_SH_MATERIALIZE=unsorted-shards` | Tip finalize alternate path. Unsorted files under `scripthash.unsorted/` |
