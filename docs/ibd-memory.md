...
| **SH unsorted-shards collect / pack** | Collect: nCPU (`RBITCOIN_SH_RECOLLECT_WORKERS` override); 1 MiB grow-on-demand write buffers; per-shard mutex so pwrite issues in offset order; 64 MiB fallocate on full flushes. Pack: min(CPUs, free RAM / 2 GiB); `RBITCOIN_SH_MERGE_WORKERS` override. One anonymous file image per pack worker (in-place unique-sort; MPHF has no HashSet). Env `RBITCOIN_SH_MATERIALIZE=unsorted-shards` | Tip finalize alternate path. Unsorted files under `scripthash.unsorted/` |
