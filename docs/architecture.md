...
- **Electrum** is **native** to the store (optional **scripthash** tables via
  ...
  **0.8 plan:** Esplora will also cover mempool/electrs `/internal/*` bulk HTTP
  so this binary can replace Core+electrs behind mempool.space `/api/`
...
| Product scope | Full node + Electrum/Esplora; **0.8 plan** Core+electrs drop-in ([`esplora-mempool-backend.md`](./esplora-mempool-backend.md)); **no** wallet/GUI/prune | Full Core product surface |
