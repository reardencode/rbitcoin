...
`--shindex`, default off; **0.8 plan** Core+electrs drop-in:
[`docs/esplora-mempool-backend.md`](./docs/esplora-mempool-backend.md)) — built
...
**0.8 (plan):** drop-in for **Bitcoin Core + mempool/electrs or Blockstream
electrs** (not address-prefix, not their `/api/v1/` Node process):
[`docs/esplora-mempool-backend.md`](./docs/esplora-mempool-backend.md).
Until that ships, Esplora is wallet-exact HTTP. Address-prefix and Liquid
stay non-goals. Product surface: [`COMPAT.md`](./COMPAT.md).
