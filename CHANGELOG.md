...
- **`list_runs`:** catalog scan does not unlink. Orphan GC is `list_runs_gc` under `RunsIoGuard`. Open-time SH `key_len` check and leftover-run **count** (catchup / tip-entry) do not delete stray `*.run`.
